...
export const Footer = () => {
  return (
    <footer id="contact" className="px-6 md:px-12 py-40 border-t border-border bg-bg relative overflow-hidden">
        ...
        <div>
          <FadeIn direction="left">
            <h2 className="font-serif text-6xl md:text-8xl lg:text-9xl leading-[0.9] tracking-tight">
              Let's create <br />
              something <br />
              <span className="italic text-rose-300">extraordinary</span>.
            </h2>
          </FadeIn>
        ...
        <div className="flex flex-col justify-between">
          <div className="grid grid-cols-2 gap-20">
            ...
            <FadeIn delay={0.5} direction="up">
              <div className="flex flex-col gap-8">
                <span className="text-[10px] uppercase tracking-[0.4em] text-muted font-bold">Navigation</span>
                <div className="flex flex-col gap-4 text-xl font-light">
                  {['Work', 'About', 'Contact'].map((item) => (
                    <a key={item} href={`#${item.toLowerCase()}`} className="hover:text-muted transition-colors w-fit relative group">
                      {item}
                      <span className="absolute -bottom-1 left-0 w-0 h-px bg-muted transition-all duration-300 group-hover:w-full" />
                    </a>
                  ))}
